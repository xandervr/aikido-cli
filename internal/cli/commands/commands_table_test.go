...
func TestSimpleListCommands_HitExpectedPath(t *testing.T) {
	...
	cases := []commandCase{
		...
		cmdCase("workspace sla-settings", NewWorkspace, []string{"sla-settings"}, "/workspace/slaSettings", "GET"),
		...
		cmdCase("containers runners", NewContainers, []string{"runners", "9"}, "/containers/9/runners", "GET"),
		...
		cmdCase("apps users", NewApps, []string{"users", "7"}, "/firewall/apps/7/users", "GET"),
		...
		cmdCase("pr-checks issue-actions", NewPRChecks, []string{"issue-actions"}, "/report/ciScans/issueActions", "GET"),
		...
		cmdCaseWithQuery("tasks projects integration", NewTasks, []string{"projects", "--integration", "5"}, "/task_tracking/projects", "GET", "integration_id=5"),
		...
		cmdCaseWithQuery("research malware-packages filtered", NewResearch, []string{"malware-packages", "--page", "1", "--per-page", "15", "--search", "evil", "--ecosystem", "npm"}, "/research/malware/packages", "GET", "filter_ecosystem=npm&page=1&per_page=15&search=evil"),
		...
		cmdCase("repos add-label", NewRepos, []string{"add-label", "3"}, "/repositories/code/3/labels", "POST"),
		cmdCase("repos update-label", NewRepos, []string{"update-label", "3", "8"}, "/repositories/code/3/labels/8", "POST"),
		cmdCase("repos remove-label", NewRepos, []string{"remove-label", "3", "8", "--confirm"}, "/repositories/code/3/labels/8", "DELETE"),
		...
		cmdCase("issues counts", NewIssues, []string{"counts"}, "/issues/counts", "GET"),
		cmdCaseWithQuery("issues bulk-detail", NewIssues, []string{"bulk-detail", "--ids", "1,2,3"}, "/issues/detail/bulk", "GET", "issue_ids=1%2C2%2C3"),
		cmdCaseWithQuery("issues bulk-detail epss", NewIssues, []string{"bulk-detail", "--ids", "1,2", "--include-epss-score"}, "/issues/detail/bulk", "GET", "include_epss_score=true&issue_ids=1%2C2"),
		cmdCase("issues notes", NewIssues, []string{"notes", "11"}, "/issues/groups/11/notes", "GET"),
		cmdCase("issues solve", NewIssues, []string{"solve", "9"}, "/issues/9/solve", "PUT"),
		...
		cmdCase("endpoint-protection devices", NewEndpointProtection, []string{"devices"}, "/endpoint-protection/devices", "GET"),
		cmdCase("endpoint-protection installed-packages", NewEndpointProtection, []string{"installed-packages"}, "/endpoint-protection/installed-packages", "GET"),
		cmdCase("endpoint-protection permission-groups", NewEndpointProtection, []string{"permission-groups"}, "/endpoint-protection/permission-groups", "GET"),
		cmdCase("endpoint-protection exceptions", NewEndpointProtection, []string{"exceptions", "npm"}, "/endpoint-protection/npm/exceptions", "GET"),
		cmdCase("endpoint-protection add-exception", NewEndpointProtection, []string{"add-exception", "npm"}, "/endpoint-protection/npm/exceptions", "POST"),
		cmdCase("endpoint-protection remove-exception", NewEndpointProtection, []string{"remove-exception", "14", "--confirm"}, "/endpoint-protection/exceptions/14", "DELETE"),
		cmdCaseWithQuery("code-quality findings", NewCodeQuality, []string{"findings", "--repo", "42", "--pr", "7"}, "/code-quality/findings", "GET", "code_repo_id=42&pr_number=7"),
...
func TestAPICatalog_CoversCurrentOpenAPISurface(t *testing.T) {
	if len(documentedEndpoints) != 161 {
		t.Fatalf("documented endpoint count = %d, want 161", len(documentedEndpoints))
	}
	want := map[string]bool{
		"GET /workspace/configurationErrors":                            false,
		"GET /workspace/slaSettings":                                    false,
		"GET /domains":                                                  false,
		"GET /localscan/latest":                                         false,
		"GET /endpoint-protection/activityLogs":                         false,
		"GET /endpoint-protection/devices":                              false,
		"GET /endpoint-protection/installed-packages":                   false,
		"GET /endpoint-protection/permission-groups":                    false,
		"GET /endpoint-protection/{ecosystem}/exceptions":               false,
		"POST /endpoint-protection/{ecosystem}/exceptions":              false,
		"DELETE /endpoint-protection/exceptions/{package_exception_id}": false,

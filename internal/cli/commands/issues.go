...
func NewIssues(g *cli.Globals) *cobra.Command {
	...
	cmd.AddCommand(
		...
		endpointCommand(g, endpointCommandConfig{Use: "notes <group-id>", Short: "List notes for issue group", Method: http.MethodGet, Args: cobra.ExactArgs(1), Path: oneArgPath("/issues/groups/%s/notes")}),
		...
		endpointCommand(g, endpointCommandConfig{Use: "solve <issue-id>", Short: "Solve an issue", Method: http.MethodPut, Args: cobra.ExactArgs(1), Path: oneArgPath("/issues/%s/solve")}),
